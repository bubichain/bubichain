...
public class ApiTestAssetGrant extends ApiGeneralCases {
	...
	public void TestProcess(Object param, JSONArray detail)
	{
		...
		if(!APIUtil.getbool(current_condition&noAssetCode))
		{
			if(APIUtil.getbool(current_condition&invalidAssetCode))
				thrMap.put("asset_code", (String)param);	
			else
				thrMap.put("asset_code", assetcode.toString());	
		}
			
		
		if(!APIUtil.getbool(current_condition&noAssetCount))
		{
			//need test limits
			if(APIUtil.getbool(current_condition&(invalidAssetCount|validAssetCount)))
				thrMap.put("asset_amount", (String)param);	
			else
