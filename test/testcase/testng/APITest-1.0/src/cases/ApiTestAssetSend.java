...
@Test
public class ApiTestAssetSend extends ApiGeneralCases {
	...
	public void TestProcess(Object param, JSONArray detail)
	{
		...
		generalSetup(current_condition,noAssetCount,invalidAssetCount,validAssetCount,"asset_amount",param,"99",thrMap);
		generalSetup(current_condition,noCurrentString,invalidCurrentString,validCurrentString,"current_string",param,APIUtil.getRandomString(32),thrMap);
	...
	private void Verification(JSONObject jsonObject)
	{
		...
		switch (current_condition)
		{
			...
			case 0:
				...
				check.equals((String)jsonObject.getJSONObject("data").get("trade_no"),trade_no,"trade_no is not as expected");
				hash =(String) jsonObject.getJSONObject("data").get("bc_hash");
				check.equals(hash.length(),64,"hash is not as expected");	
				Confirmation(true,hash);
				break;
	...
	public void Confirmation(boolean issuccess,String ... param)
	{
		...
		System.out.println("=========check send status========");
		String result = checkStatus("/status/asset/v1/send",access_token,trade_no,null);
		JSONObject jsonObject =JSONObject.fromObject(result);  
		if(issuccess)
		{
			check.equals(jsonObject.getString("err_code"), "0", "errorcode is not as expected");
			check.equals((String) jsonObject.getJSONObject("data").get("bc_hash"),param[0],"hash is not as expected");
			check.equals((String) jsonObject.getJSONObject("data").get("trade_no"),trade_no,"trade_no is not as expected");	
	...
	{
		assetCodeTest(noAssetCode,invalidAssetCode,validAssetCode);
	}
